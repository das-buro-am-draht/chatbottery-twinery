...
const HomeInfoContent = Vue.extend({
	...
	methods: {
		...
		transformContent(_content) {
			return _content.replace(/(<([^>]+)>)/gi, "").trim();
		},
		createStoryPrompt(e) {
			...
			prompt({
				...
				// const id = this.existingStories.find((story) => story.name === name).id;
				...
				// window.setTimeout(() => {
				// 	this.$root.$emit('story-edit', this.existingStories.find(story => story.name === name)
				// 	.id);
				// }, 300);
			});
		},
