...
import { latestFormatVersions, formatVersion } from '../format-versions';
import locale from '../../locale';
import notify from '../../ui/notify';
import {setPref} from './pref';
...
export const createFormat = ({commit}, props) => {
	commit('CREATE_FORMAT', props);
};

export const updateFormat = ({commit}, props) => {
	commit('UPDATE_FORMAT', props);
};

export const deleteFormat = ({commit}, id) => {
	commit('DELETE_FORMAT', id);
};

export const createFormatFromUrl = (store, url) => {
...
export const loadFormat = (store, {name, version}) => {
	/*
...
export const repairFormats = (store) => {
	/*
...
*/		{
	...
	builtinFormats.forEach(builtin => {
		...
		if (format) {
			updateFormat(store, {id: format.id, builtin});
		} else {
	...
	*/
	...
	store.state.storyFormat.formats.forEach(format => {
		...
		if (v.semver !== latestVersions[format.name][v.major].semver) {
			...
			updateFormat(store, { id: format.id, userAdded: true });
		}
	});
